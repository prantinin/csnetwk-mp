...
1. Make sure you are in the root folder: 
   `\csnetwk-mp`
2. Open a terminal to the project directory and run the host file using this command  
    `python -m pokeprotocol.host`
3. You can run the joiner and spectator files by opening separate terminals and using one of these commands  
    `python -m pokeprotocol.joiner`  
4. If you want to see the debug texts:
   `python -m pokeprotocol.joiner --verbose`  
   `python -m pokeprotocol.host --verbose`
